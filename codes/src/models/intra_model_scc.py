...
from src.models.color_utils.color_cnn import PaletteCNN
from src.models.our_utils import AggregateAttentionBlock
...
class IntraModel(CompressionModel):
    ...
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=64,  M=320, num_slices=5, max_support_slices=5, **kwargs):
        ...
        begin = 0
        # self.m_down0 = ResidualBlockWithStride(3, 2*N, 2)
        ...
        self.m_up0 = ResidualBlockUpsample(M, 2*N, 2)
        ...
        self.g_a0 = self.g_a[0]
        self.g_a1 = nn.Sequential(*self.m_down1)
        self.g_a2 = nn.Sequential(*self.m_down2)
        self.g_a3 = nn.Sequential(*self.m_down3)
        self.g_s0 = self.g_s[0]
        self.g_s1 = nn.Sequential(*self.m_up1)
        self.g_s2 = nn.Sequential(*self.m_up2)
        self.g_s3 = nn.Sequential(*self.m_up3)

        self.ga_rba1 = AggregateAttentionBlock(2*N,False)
        self.ga_rba2 = AggregateAttentionBlock(2*N,False)
        self.gs_rba1 = AggregateAttentionBlock(2*N,True)
        self.gs_rba2 = AggregateAttentionBlock(2*N,True)
        ...
        self.block_size = 128
        self.color_nums = 3
        self.pltcnn = PaletteCNN(num_colors=self.color_nums)

        self.cm0 = nn.Sequential(conv3x3(3 * self.color_nums, M),nn.LeakyReLU(inplace=True))
        self.cm1 = nn.Sequential(subpel_conv3x3(M, 2*N, 2), nn.LeakyReLU(inplace=True))
        self.cm2 = nn.Sequential(subpel_conv3x3(2*N, 2*N, 2), nn.LeakyReLU(inplace=True))
        self.cm3 = nn.Sequential(subpel_conv3x3(2*N, 2*N, 2), nn.LeakyReLU(inplace=True))
    ...
    def update(self, scale_table=None, force=False):
        ...
        return updated
    def g_a_func(self, x, cm_list):
        out = self.g_a0(x)+cm_list[3]
        out = self.g_a1(out)+cm_list[2]
        out = self.ga_rba1(out)
        out = self.g_a2(out)+cm_list[1]
        out = self.ga_rba2(out)
        out = self.g_a3(out)+cm_list[0]
        return out

    def g_s_func(self, x, cm_list):

        out = self.g_s0(x+cm_list[0])
        out = self.gs_rba1(out)
        out = self.g_s1(out+cm_list[1])
        out = self.gs_rba2(out)
        out = self.g_s2(out+cm_list[2])

        out = self.g_s3(out+cm_list[3])

        return out

    def forward(self, x):
        b, c, h, w = x.shape
        patch_list_tensor = pic2cu(x, self.block_size)
        color_palette_list = []
        for idx in range(patch_list_tensor.shape[1] // 3):
            patch_tensor = patch_list_tensor[:, 3 * idx:3 * (idx + 1), :, :]
            _, color_palette = self.pltcnn(patch_tensor)  # B,3,n_color,1,1
            color_palette_list.append(
                color_palette.reshape(color_palette.shape[0], -1, color_palette.shape[-2], color_palette.shape[-1]))

        color_map = torch.zeros((b, c * self.color_nums, h // self.block_size, w // self.block_size)).to(x)
        color_map = cu2pic(torch.cat(color_palette_list, dim=1), color_map, 1, c * self.color_nums)

        # color_map = (color_map*1000).round()/1000
        color_map = color_map.repeat_interleave(self.block_size//(2**4),2).repeat_interleave(self.block_size//(2**4),3)
        cm_list = []
        cm0 = self.cm0(color_map)
        cm1 = self.cm1(cm0)
        cm2 = self.cm2(cm1)
        cm3 = self.cm3(cm2)
        cm_list.append(cm0)
        ...
        x_hat = self.g_s_func(y_hat,cm_list)
    ...
    def forward_round(self, x):
        ...
        color_map = (color_map * 255).round()
        calc_real_bits = True
        if calc_real_bits:
            from src.models.torchac_bits import calc_tensor_bits
            plt_bits = calc_tensor_bits(color_map.reshape(1,-1).cpu().to(dtype=torch.int16))
    ...
    def from_state_dict(cls, state_dict):
        ...
        N = 128#state_dict["g_a.0.weight"].size(0)
        M = 320#state_dict["g_a.6.weight"].size(0)
        # net = cls(N, M)
        net = cls()
...
if __name__=="__main__":
    ...
    net = model
    # net = net.cpu()
    macs, params = get_model_complexity_info(net, (3, 256, 256), as_strings=True, verbose=True)
